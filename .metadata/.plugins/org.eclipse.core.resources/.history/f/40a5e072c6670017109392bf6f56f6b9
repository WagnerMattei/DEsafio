...
public class TransacaoService
{
	...
	public ResponseEntity<String> update(Transacao transacao)
	{
		...
		if (transacao.getDataVencimento().before(Calendar.getInstance()) && !transacao.isEfetivado())
		{
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("A data de vencimento deve ser maior que a data atual");
		}
		...
		if (transacao.getFilePath() == null)
		{
			this.excluirArquivo(transacao);		
		}
	...
	{
		System.out.println(transacaoRepository.findOne(id).getFilePath());
    	String path = transacaoRepository.findOne(id).getFilePath();
		arquivo.read(response, id, path);
	}
	
	public ResponseEntity<String> excluirArquivo(Transacao transacao)
	{
		if (transacao.equals(null))
		{
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Transação nao possui comprovante");
		}
		transacao.setFilePath(null);
		transacaoRepository.saveAndFlush(transacao);
		return ResponseEntity.status(HttpStatus.ACCEPTED).body("Comprovante removido com sucesso!");
	}
